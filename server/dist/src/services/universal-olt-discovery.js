...
async function etapa1_identidade(session) {
    log('\n' + '='.repeat(80));
    ...
    return new Promise((resolve) => {
        ...
        session.get(oids, (error, varbinds) => {
            ...
            log(`snmpget -v2c -c ${COMMUNITY} ${OLT_IP} 1.3.6.1.2.1.1.1.0 1.3.6.1.2.1.1.2.0`);
            resolve();
        });
    });
}
async function etapa2_mapearPons(session) {
    log('\n' + '='.repeat(80));
    ...
    return new Promise((resolve) => {
        ...
        const interfaces = [];
        session.walk(IF_DESCR_OID, 100, (varbinds) => {
            varbinds.forEach((vb) => {
            ...
            log('\n📝 Comando para repetir:');
            log(`snmpwalk -v2c -c ${COMMUNITY} ${OLT_IP} 1.3.6.1.2.1.2.2.1.2 | grep -i pon`);
            resolve();
        });
    });
}
async function etapa3_uplinkPower(session) {
    log('\n' + '='.repeat(80));
    ...
    return new Promise((resolve) => {
        ...
        session.walk(ENTITY_SENSOR_OID, 50, (varbinds) => {
            varbinds.forEach((vb) => {
                if (!snmp.isVarbindError(vb)) {
                    const value = vb.value?.toString().toLowerCase() || '';
                    if (value.includes('optical') ||
            ...
            log('\n📝 Comando para repetir:');
            log(`snmpwalk -v2c -c ${COMMUNITY} ${OLT_IP} 1.3.6.1.2.1.99 | grep -iE "optical|power|tx|rx"`);
            resolve();
        });
    });
}
async function etapa4_onuPower(session) {
    log('\n' + '='.repeat(80));
    ...
    return new Promise((resolve) => {
        ...
        let completed = 0;
        const highCardinalityTables = [];
        searchTrees.forEach((tree) => {
            const results = [];
            session.walk(tree.oid, 100, (varbinds) => {
                varbinds.forEach((vb) => {
                    if (!snmp.isVarbindError(vb)) {
...
function gerarRelatorioFinal() {
    ...
    if (report.ponInterfaces.length > 0) {
        log(`✅ PORTAS PON: ${report.ponInterfaces.length} identificadas via IF-MIB`);
        report.conclusion.push(`PON interfaces: ${report.ponInterfaces.length} via IF-MIB`);
    }
    else {
        log('❌ PORTAS PON: Não identificadas automaticamente');
        report.conclusion.push('PON interfaces: Não identificadas (verificar manualmente)');
    }
    if (report.uplinkPower.found) {
        log(`✅ POTÊNCIA UPLINK: Disponível via SNMP (${report.uplinkPower.oids.length} sensores)`);
...
//# sourceMappingURL=universal-olt-discovery.js.map